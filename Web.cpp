...
void Web::begin() {
  ...
  server.on("/logo-somfy-test0.png", []() { webServer.sendCacheHeaders(604800); webServer.handleStreamFile(server, "/logo-somfy-test0.png", "image/png"); });
  server.on("/icon.jpg", []() { webServer.sendCacheHeaders(604800); webServer.handleStreamFile(server, "/icon.jpg", "image/png"); });
